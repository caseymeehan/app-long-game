...
describe("courseService", () => {
  ...
  describe("getAllCourses", () => {
    ...
    it("returns multiple courses", async () => {
      await createCourse("Second", "second", "desc", base.instructor.id, base.category.id, null);
      const all = await getAllCourses();
  ...
  describe("updateCourse", () => {
    it("updates title and description", async () => {
      const updated = await updateCourse(base.course.id, "Updated Title", "Updated description");
    ...
    it("sets updatedAt to a new timestamp", async () => {
      ...
      const updated = await updateCourse(base.course.id, "New Title", "New desc");
  ...
  describe("getPublishedCourses", () => {
    it("returns only published courses", async () => {
      await createCourse("Draft", "draft", "desc", base.instructor.id, base.category.id, null);
  ...
  describe("updateCourseStatus", () => {
    ...
    it("transitions from draft to published", async () => {
      const draft = await createCourse("Draft", "draft", "desc", base.instructor.id, base.category.id, null);
  ...
  describe("buildCourseQuery", () => {
    it("returns all courses when no filters applied", async () => {
      const results = await buildCourseQuery(null, null, null, null, 10, 0);
    ...
    it("filters by search term in title", async () => {
      await createCourse("JavaScript Basics", "js-basics", "Learn JS", base.instructor.id, base.category.id, null);

      const results = await buildCourseQuery("JavaScript", null, null, null, 10, 0);
    ...
    it("filters by search term in description", async () => {
      await createCourse("Intro", "intro", "Learn Python programming", base.instructor.id, base.category.id, null);

      const results = await buildCourseQuery("Python", null, null, null, 10, 0);
      expect(results).toHaveLength(1);
      expect(results[0].title).toBe("Intro");
    });

    it("filters by status", async () => {
      await createCourse("Draft", "draft-c", "desc", base.instructor.id, base.category.id, null);

      const published = await buildCourseQuery(null, null, schema.CourseStatus.Published, null, 10, 0);
      expect(published).toHaveLength(1);
      expect(published[0].status).toBe(schema.CourseStatus.Published);

      const drafts = await buildCourseQuery(null, null, schema.CourseStatus.Draft, null, 10, 0);
    ...
    it("filters by category slug", async () => {
      const [designCat] = await testDb
        ...
        .returning();
      await createCourse("Design 101", "design-101", "desc", base.instructor.id, designCat.id, null);

      const results = await buildCourseQuery(null, "design", null, null, 10, 0);
      expect(results).toHaveLength(1);
      expect(results[0].title).toBe("Design 101");
    });

    it("combines search and status filters", async () => {
      await createCourse("Test Draft", "test-draft", "desc", base.instructor.id, base.category.id, null);

      const results = await buildCourseQuery("Test", null, schema.CourseStatus.Published, null, 10, 0);
      expect(results).toHaveLength(1);
      expect(results[0].title).toBe("Test Course");
    });
    ...
    it("respects limit", async () => {
      await createCourse("Second", "second", "desc", base.instructor.id, base.category.id, null);
      await createCourse("Third", "third", "desc", base.instructor.id, base.category.id, null);

      const results = await buildCourseQuery(null, null, null, null, 2, 0);
    ...
    it("respects offset", async () => {
      ...
      const all = await buildCourseQuery(null, null, null, null, 10, 0);
      const offset = await buildCourseQuery(null, null, null, null, 10, 1);
      expect(offset).toHaveLength(all.length - 1);
    });

    it("sorts by title", async () => {
      await createCourse("Alpha Course", "alpha", "desc", base.instructor.id, base.category.id, null);
      await createCourse("Zeta Course", "zeta", "desc", base.instructor.id, base.category.id, null);

      const results = await buildCourseQuery(null, null, null, "title", 10, 0);
    ...
    it("returns empty array when no courses match", async () => {
      const results = await buildCourseQuery("nonexistent-query-xyz", null, null, null, 10, 0);
